...
sync_cursor() {
    echo -e "\n${BLUE}Cursor IDE (.cursor/rules/)${NC}"
    echo -e "${BLUE}---------------------------${NC}"

    local rules_dest="${DOTFILES_ROOT}/.cursor/rules"
    mkdir -p "${rules_dest}"

    # 1) Project rules: .cursor/rules/project/RULE.md -> RULES.md
    mkdir -p "${rules_dest}/project"
    ensure_symlink \
        "${rules_dest}/project/RULE.md" \
        "../../../RULES.md" \
        "project/RULE.md" \
        "RULES.md"

    # 2) Agents: .cursor/rules/agents/<name>/RULE.md -> templates/.../subagents/<name>.md
    local agent_source="${TEMPLATE_AGENT}/subagents"
    if [[ -d "${agent_source}" ]]; then
        mkdir -p "${rules_dest}/agents"
        for agent_file in "${agent_source}"/*.md; do
            if [[ -f "${agent_file}" ]]; then
                local agent_name
                agent_name="$(basename "${agent_file}" .md)"
                local agent_dir="${rules_dest}/agents/${agent_name}"
                mkdir -p "${agent_dir}"

                ensure_symlink \
                    "${agent_dir}/RULE.md" \
                    "../../../../templates/project/dot.agent/subagents/${agent_name}.md" \
                    "agents/${agent_name}/RULE.md" \
                    "...subagents/${agent_name}.md"
            fi
        done
    fi

    # 3) Skills: .cursor/rules/skills/<name>/RULE.md -> templates/.../skills/<name>.md
    local skill_source="${TEMPLATE_AGENT}/skills"
    if [[ -d "${skill_source}" ]]; then
        mkdir -p "${rules_dest}/skills"
        for skill_file in "${skill_source}"/*.md; do
            if [[ -f "${skill_file}" ]]; then
                local skill_name
                skill_name="$(basename "${skill_file}" .md)"
                local skill_dir="${rules_dest}/skills/${skill_name}"
                mkdir -p "${skill_dir}"

                ensure_symlink \
                    "${skill_dir}/RULE.md" \
                    "../../../../templates/project/dot.agent/skills/${skill_name}.md" \
                    "skills/${skill_name}/RULE.md" \
                    "...skills/${skill_name}.md"
            fi
        done
    fi
...
sync_codex() {
    echo -e "\n${BLUE}OpenAI Codex (.codex/)${NC}"
    echo -e "${BLUE}----------------------${NC}"

    local codex_dest="${DOTFILES_ROOT}/.codex"
    mkdir -p "${codex_dest}"

    # 1) Skills: .codex/skills/<name>/SKILL.md -> templates/.../skills/<name>.md
    local skill_source="${TEMPLATE_AGENT}/skills"
    if [[ -d "${skill_source}" ]]; then
        local skills_dest="${codex_dest}/skills"
        mkdir -p "${skills_dest}"
        ...
        for skill_file in "${skill_source}"/*.md; do
            if [[ -f "${skill_file}" ]]; then
                local skill_name
                skill_name="$(basename "${skill_file}" .md)"
                local skill_dir="${skills_dest}/${skill_name}"
                mkdir -p "${skill_dir}"
    ...
    # 2) Generate AGENTS.override.md from subagents
    local agent_source="${TEMPLATE_AGENT}/subagents"
    local override_file="${codex_dest}/AGENTS.override.md"
...
HEADER
            ...
            log_fixed "AGENTS.override.md" "generated (${agent_count} agents)"
        else
            if [[ -f "${override_file}" ]]; then
                log_ok "AGENTS.override.md" "exists"
            else
                log_warn "AGENTS.override.md" "missing (will be generated)"
            fi
        fi
    fi
}
